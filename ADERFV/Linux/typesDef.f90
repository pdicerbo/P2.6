MODULE typesDef
  ...
  INTEGER            :: IMAX, NMAX                          ! The number of cells in each space dimension & max. number of time steps 
  ...
  INTEGER, POINTER   :: tri(:,:), neighbor(:,:)             ! connectivity from the element to the nodes 
  INTEGER, POINTER   :: idxe0(:)
  INTEGER, ALLOCATABLE :: BNDstatus(:) 
  ...
  !
  ! The main variables of the ADER-FV scheme 
  !  
  REAL, POINTER      :: uh(:,:)                       ! the cell averages of the FV scheme        (nVar, 1, nElem) 
  REAL, POINTER      :: duh(:,:)                      ! the update cell averages of the FV scheme (nVar, 1, nElem) 
  ...
  REAL, POINTER      :: wh(:,:,:)                       ! the coefficients of the WENO polynomial        (nVar, nDOF(1), nElem) 
  !
  ! WENO Variables 
  INTEGER, PARAMETER :: ss = MAX(2,CEILING(REAL(N+1)/REAL(1))-1)
  REAL, PARAMETER    :: WENO_r        = 8
  REAL, PARAMETER    :: WENO_epsilon  = 1e-14 
  REAL               :: WENO_lambdaC  = 1e5   
  INTEGER            :: WENO_left(4), WENO_right(4), WENO_nStencil   
  REAL               :: WENO_lambda(4)
  REAL, ALLOCATABLE  :: WENO_iMatrix(:,:,:),WENO_iMatrixPrim(:,:,:)
  REAL               :: WENO_OIMatrix(N+1,N+1)
  !
